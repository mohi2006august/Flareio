...
function AlertLogEntry({ entry }) {
  const cfg = ALERT_LEVELS[entry.level];
  return (
    <div className="log-entry" style={{ '--entry-color': cfg?.color || '#ffffff' }}>
...
function CatalogueEntry({ entry }) {
  ...
  return (
    <div className="log-entry" style={{ '--entry-color': color }}>
      <div className="log-entry__time">{formatTimestamp(entry.timestamp)}</div>
      <div className="log-entry__body">
  ...
  return (
    <div className="log-panels-row">
      <LogPanel title="ALERT LOG" icon="📋" count={state.alertLog.length}>
        {state.alertLog.map(entry => (
          <AlertLogEntry key={entry.id} entry={entry} />
        ))}
      </LogPanel>
