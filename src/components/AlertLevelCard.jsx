    ...
    if (prevLevelRef.current !== alertLevel && (alertLevel === 'ORANGE' || alertLevel === 'RED')) {
      if (cardRef.current) {
        cardRef.current.animate(
          [
            { transform: 'scale(1)', boxShadow: `0 0 0px ${cfg.color}` },
            { transform: 'scale(1.02)', boxShadow: `0 0 40px ${cfg.color}88` },
            { transform: 'scale(1)', boxShadow: `0 0 20px ${cfg.color}44` },
          ],
  ...
  return (
    <div
      ...
      style={{
        ...
        '--alert-glow': cfg.glow,
        boxShadow: `0 0 30px ${cfg.glow}, 0 8px 32px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.1)`,
      }}
    >
      ...
      <div className="alert-card__main">
        <div
          ...
          style={{ color: cfg.color, textShadow: `0 0 20px ${cfg.color}88` }}
        >
          {alertLevel}
        </div>
        <div className="alert-card__flare-class">
          <span className="flare-class-label">FLARE CLASS</span>
          <span className="flare-class-value" style={{ color: cfg.color }}>
            {flareClass}
          </span>
        </div>
      </div>
